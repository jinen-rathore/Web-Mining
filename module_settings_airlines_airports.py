...
### MODULE_SETTINGS_AIRLINES_AIRPORTS.PY

### This is the settings airlines and airports module. In here you can find several functions:

### - Functions to calculate the degree of the airports
### - Functions to select the right dataframes from specific/biggest airlines/airports for input
### - Functions to define the user input for specific airlines/airports
...
def create_graph_object(df):
    graph = nx.from_pandas_edgelist(df, source = 'source airport', target = 'destination airport')
    return graph
...
#%% Metafunctions for airports/hubs
